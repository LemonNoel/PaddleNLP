...
def convert_tnews(example):
    # ('金融', 267), ('教育', 206), ('娱乐', 194), ('科技', 154), ('旅游', 151), ('文学', 141), ('军事', 127), ('游戏', 125), ('故事', 118), ('体育', 113), ('房产', 112), ('世界', 110), ('汽车', 95), ('农业', 81), ('股票', 16)
    label_list = [
        '故事', '文学', '娱乐', '体育', '财经', '房产', '汽车', '教育', '科技', '军事', '旅游', '世界',
        '金融', '农业', '游戏'
    ]
    return InputExample(uid=example["id"],
                        text_a="“" + example["sentence"] + "”是什么新闻频道写的？选项：" +
                        "，".join(label_list) + "。答：",
                        text_b="",
...
def convert_iflytek(example):
    return InputExample(uid=example["id"],
                        text_a="“" + example["sentence"] + "”上述句子属于什么类别？选项：" +
                        "，".join(list(LABEL_MAP["iflytek"].values())) + "。答：",
                        text_b="",
...
def convert_ocnli(example):
    ...
    return InputExample(uid=example["id"],
                        ...
                        example["sentence2"] + "”之间的逻辑关系是什么？答：",
                        text_b="",
                        labels=example.get("label", None))
...
def convert_bustm(example):
    ...
    return InputExample(uid=example["id"],
                        text_a='“' + example["sentence1"] + '”和“' +
                        example["sentence2"] + '”这两句话是在说同一件事吗?',
                        text_b=None,
                        labels=example.get("label", None))


def A_convert_chid(example):
    # Unlabeled: 7585
...
def convert_chid(example):
    return InputExample(uid=example["id"],
                        text_a="问题：“" + example["content"] +
                        "”。此问题的正确答案是什么？选项：" + "，".join(example["candidates"]) +
                        "。",
                        text_b=None,
                        labels=example["candidates"][example.get("answer")])
...
def convert_csl(example):
    ...
    return InputExample(uid=example["id"],
                        text_a="“" + example["abst"] + "”和“" +
                        "，".join(example["keyword"]) + '”这两句话是在说同一件事吗？',
                        text_b="",
                        labels=example.get("label", None))
...
def A_convert_cluewsc(example):
    ...
    return InputExample(uid=example.get("id", None),
                        text_a="“" + example["text"] + "”中" +
                        example["target"]["span2_text"] + "指的是" +
                        example["target"]["span1_text"] + "这里" +
                        example["target"]["span2_text"] + "的指代是正确的吗？",
                        text_b="",
                        labels=example.get("label", None))
...
def convert_labels_to_ids(example, label_dict):
    if example.labels is not None and label_dict is not None:
