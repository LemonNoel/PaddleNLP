...
from paddlenlp.prompt import PromptTrainer, PromptModelForClassification, InputExample
...
class DataArguments:
    ...
    #     default=512,
    #     metadata={"help": "The maximum length of all input text."}
    # )
    ...
    #     default="tail",
    #     metadata={
    #         "help": "How to truncate input text. Truncate input text "\
    #                 "according to `first_max_length` and `other_max_length`"\
    #                 "in the manual mode."
    #     }
    # ) #Literal["tail", "head", "manual"]
...
def main():
    ...
    verbalizer = SoftVerbalizer.from_file(tokenizer,
                                          model,
                                          os.path.join(data_args.data_path,
                                                       'label.txt'),
                                          prefix=' ')
    ...
    #model = verbalizer.process_model(model)
    #print(model)
    ...
    #     learning_rate=3e-4,
    #     parameters=[
    #         {'params': verbalizer.head_parameters()},
    #         {'params': verbalizer.non_head_parameters(), 'learning_rate': 0.1},
    #     ]
    # )

    optimizer = paddle.optimizer.AdamW(
        learning_rate=training_args.ppt_learning_rate,
        parameters=prompt_model.parameters())
